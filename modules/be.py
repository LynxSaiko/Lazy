...
OPTIONS = {
    ...
    "wordlist": {
        "type": "str",
        "description": "Path to wordlist file or built-in list (cms or non-cms)",
        "required": False,
        "default": "common"
    },
    "extensions": {
    ...
    "cms": {
        "type": "bool",
        "description": "If true, use CMS specific wordlist",
        "required": False,
        "default": True
    }
}
...
# CMS and Non-CMS Wordlists
def get_cms_wordlist():
    """Comprehensive wordlist for CMS (WordPress, Joomla, Drupal, etc.)"""
    wordlist = [
        "wp-admin", "wp-content", "wp-includes", "wordpress",
        "phpmyadmin", "mysql", "pma",  # phpMyAdmin for database access
        "joomla", "drupal", "magento", "prestashop",
        "laravel", "symfony", "yii", "codeigniter",
        "wp-config.php", "configuration.php", ".env", ".git",
        "robots.txt", "sitemap.xml", "sitemap.html", "readme.txt", "license.txt"
    ]
    return wordlist


def get_non_cms_wordlist():
    """Comprehensive wordlist for Non-CMS websites"""
    wordlist = [
        ...
        "uploads", "downloads", "files", "storage", "media", "doc", "docs",
        "backup", "backups", "old", "temp", "tmp", "cache", "log", "logs", "db",
        "database", "sql", "data", "config", "configuration", "setup", "install",
        ...
        "backup.zip", "dump.sql", "database.sql", "backup.sql", "test.php",
        "info.php", "phpinfo.php", "debug.php",
...
def load_wordlist(wordlist_param, cms=True):
    """Load wordlist from file or use built-in CMS or Non-CMS wordlist"""
    
    # Check if the wordlist_param is a file with .txt extension
    if wordlist_param.endswith(".txt"):
        if os.path.isfile(wordlist_param):  # Check if the file exists
            try:
                with open(wordlist_param, 'r', encoding='utf-8', errors='ignore') as f:
                    words = [line.strip() for line in f if line.strip() and not line.startswith('#')]
                    print(f"[+] Loaded wordlist from '{wordlist_param}'")
                    return words  # Return words from the .txt file
            except Exception as e:
                print(f"[!] Error loading wordlist from file '{wordlist_param}': {e}")
                return get_cms_wordlist() if cms else get_non_cms_wordlist()
        else:
            print(f"[!] File '{wordlist_param}' does not exist. Using default wordlist.")
            return get_cms_wordlist() if cms else get_non_cms_wordlist()

    # If user provided "common", use the built-in wordlist
    if wordlist_param == "common":
        return get_cms_wordlist() if cms else get_non_cms_wordlist()
    
    # For "large", generate variations and load
    elif wordlist_param == "large":
        if cms:
            large_list = get_cms_wordlist()
        else:
            large_list = get_non_cms_wordlist()
    ...
    # If none of the above, use default wordlist
    else:
        return get_cms_wordlist() if cms else get_non_cms_wordlist()
...
def run(session, options):
    ...
    cms = options.get("cms", True)  # Set CMS option
    ...
    base_words = load_wordlist(wordlist_param, cms)  # Use cms flag to determine which list to load
